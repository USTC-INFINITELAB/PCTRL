...
class ofp_features_reply (ofp_header):
  ...
  def unpack (self, raw, offset=0):
    ...
    print('offset:',offset)  #cc
    return offset,length
  """
  def unpack (self, raw, offset=0):
    offset,length = self._unpack_header(raw, offset)
    #offset,(self.datapath_id, self.n_buffers, self.n_tables) = _unpack("!QLB", raw, offset)
    offset,(self.datapath_id, self.capabilities) = _unpack("!QL", raw, offset)
    
    offset = _skip(raw, offset, 4)
    #offset,(self.capabilities, self.actions) = _unpack("!LL", raw, offset)
    #portCount = (length - 32) / len(ofp_phy_port)
    portCount = (length - 24) / 64
    self.ports = []
    for i in xrange(0, portCount):
      #p = ofp_phy_port()
      #offset = p.unpack(raw, offset)
      p = raw[offset:offset+64]
      offset += 64
      self.ports.append(p)
    print (len(self))
    print (length)
  ...
  def __len__ (self):
    #print ('self.ports:',len(self.ports))  # cc
